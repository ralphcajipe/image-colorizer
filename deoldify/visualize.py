...
class ModelImageVisualizer:
    ...
    def get_transformed_image(
            self, path: Path, render_factor: int = None,
            post_process: bool = True,
            watermarked: bool = True,
    ) -> Image:
        self._clean_mem()
...
class VideoColorizer:
    ...
    def _extract_raw_frames(self, source_path: Path):
        ...
        process = (
            ...
            .output(str(bwframe_path_template), format='image2',
                    vcodec='mjpeg', **{'q:v': '0'})
    ...
    def _colorize_raw_frames(
            self, source_path: Path, render_factor: int = None,
            post_process: bool = True,
            watermarked: bool = True,
    ):
        ...
        for img in progress_bar(os.listdir(str(bwframes_folder))):
            ...
            if os.path.isfile(str(img_path)):
                color_image = self.vis.get_transformed_image(
                    ...
                    post_process=post_process, watermarked=watermarked
                )
    ...
    def colorize_from_url(
        ...
        return self._colorize_from_path(
            source_path, render_factor=render_factor,
            post_process=post_process, watermarked=watermarked
        )

    def colorize_from_file_name(
            self, file_name: str, render_factor: int = None,
            watermarked: bool = True, post_process: bool = True,
    ) -> Path:
        source_path = self.source_folder / file_name
        return self._colorize_from_path(
            source_path, render_factor=render_factor,
    ...
    def _colorize_from_path(
        ...
        if not source_path.exists():
            raise Exception(
                'Video at path specfied, ' + str(
                    source_path) + ' could not be found.'
            )
...
def get_image_colorizer(
        root_folder: Path = Path('./'), render_factor: int = 35,
        artistic: bool = True
) -> ModelImageVisualizer:
    if artistic:
        return get_artistic_image_colorizer(root_folder=root_folder,
                                            render_factor=render_factor)
    else:
        return get_stable_image_colorizer(root_folder=root_folder,
                                          render_factor=render_factor)
